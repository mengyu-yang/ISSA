...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    if os.path.isfile(meta_fname):
        with open(meta_fname, 'r') as file:
            ...
            if labels is not None:
                ...
                for x in labels:
                    x_split = x.split(" ")
                    all_labels.append(int(x_split[1]))
                labels_distinct = list(set(all_labels))[:1000]
                label_to_idx = {}
                for k in range(len(labels_distinct)):
                    label_to_idx[labels_distinct[k]] = k

                for x in labels:
                    x_split = x.split(" ")
                    # format_labels[re.sub('-','', x_split[0]+'.jpg')] = label_to_idx.get(int(x_split[1]))
                    format_labels[re.sub('-', '', x_split[0])] = label_to_idx.get(int(x_split[1]))
            else:
                format_labels = {}
    ...
    def iterate_images():
        for idx, fname in enumerate(input_images):
            ...
            yield dict(img=img, label=format_labels.get(arch_fname))
            if idx >= max_idx-1:
                break
...
def convert_dataset(
    ...
    with open('/scratch/ssd002/home/mengyu/stylegan2-ada-pytorch/few_shot_data/dataset.json', 'w') as outfile:
