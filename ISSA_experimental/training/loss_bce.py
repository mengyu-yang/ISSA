    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, nimg):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    # print(real_img.shape, real_c.shape)
                    # label = [torch.argmax(real_c[i]) for i in range(32)]
                    # print(label)
                    ...
                    # Calculate gradient of generator output wrt input
                    context_grads = torch.zeros(context.shape[0])
                    for i in range(context.shape[0]):
                        context_grads[i] = torch.autograd.grad(gen_img[i, ...].sum(), context, retain_graph=True)[0][i].abs().mean()
                    training_stats.report('Grad/d_gen_img-d_context', context_grads.mean())

                    noise_grads = torch.zeros(context.shape[0])
                    for i in range(context.shape[0]):
                        noise_grads[i] = torch.autograd.grad(gen_img[i, ...].sum(), gen_z, retain_graph=True)[0][i].abs().mean()
                    training_stats.report('Grad/d_gen_img-d_noise', noise_grads.mean())

                    # Minimize encoder logits for real images
                    enc_logits = self.run_D(real_img, context, sync=False)
                    label = torch.full_like(enc_logits, self.fake_label, dtype=torch.float, device=self.device)
                    loss_E = self.criterion(enc_logits, label)
