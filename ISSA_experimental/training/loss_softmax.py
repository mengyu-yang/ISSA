...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, use_softmax=True, shuffledisc=True, concatlabel=False, augment_pipe=None, E=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        super().__init__()
        self.device = device
    ...
    def run_E(self, img, sync):
        with misc.ddp_sync(self.E, sync):
            with torch.no_grad():
                ...
                feature_map = feature_map.repeat_interleave(ssize, dim=0)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, ssize_mult):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    ...
                    if self.concatlabel:
                        context = torch.cat((context, real_c), dim=1)
                        feature_map = torch.cat((feature_map, real_c), dim=1)
                    ...
                    if self.use_softmax:
                        ...
                        gen_img, _gen_ws = self.run_G(gen_z, feature_map, sync=(sync and not do_Gpl))  # May get synced by Gpl.
                        gen_logits = self.run_D(gen_img, feature_map, sync=False)
                else:
        ...
        if do_Gpl:
            ...
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                if self.encode:
                    context, feature_map, _ = self.run_E(real_img, sync=sync)
                    ...
                    if self.use_softmax:
                        ...
                        gen_img, gen_ws = self.run_G(gen_z[:batch_size], feature_map[:batch_size], sync=sync)
                else:
            ...
            with torch.autograd.profiler.record_function('Dgen_forward'):
                if self.encode:
                    ...
                    if self.use_softmax:
                        ...
                        gen_img, _gen_ws = self.run_G(gen_z, feature_map, sync=False)
                        gen_logits = self.run_D(gen_img, feature_map, sync=False)  # Gets synced by loss_Dreal
                else:
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if self.encode:
                    ...
                    if self.use_softmax:
                        if self.shuffledisc:
                            real_logits = self.run_D_shuffle(real_img_tmp, context, ssize, sync=sync, ssize_mult=ssize_mult)
                        else:
                            real_logits = self.run_D(real_img_tmp, context, sync=sync)

                    else:
                        if self.shuffledisc:
                            real_logits = self.run_D_shuffle(real_img_tmp, feature_map, ssize, sync=sync, ssize_mult=ssize_mult)
                        else:
                            real_logits = self.run_D(real_img_tmp, feature_map, sync=sync)
