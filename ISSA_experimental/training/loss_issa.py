    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, nimg):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    context, feature_map, ssize = self.run_E(real_img, sync=(sync and not do_Gpl))
                ...
                loss_Emain = torch.nn.functional.softplus(enc_logits)

                training_stats.report('Loss/G/loss', loss_Gmain)
            with torch.autograd.profiler.record_function('Gmain_backward'):
                (loss_Gmain + loss_Emain).mean().mul(gain).backward()
