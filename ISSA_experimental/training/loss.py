            ...
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    ...
                    for i in range(context.shape[0]):
                        context_grads[i] = torch.autograd.grad(gen_img[i, ...].sum(), context, retain_graph=True)[0][
                            i].abs().mean()
                    training_stats.report('Grad/d_gen_img-d_context', context_grads.mean())

                    noise_grads = torch.zeros(context.shape[0])
                    for i in range(context.shape[0]):
                        noise_grads[i] = torch.autograd.grad(gen_img[i, ...].sum(), gen_z, retain_graph=True)[0][
                            i].abs().mean()
