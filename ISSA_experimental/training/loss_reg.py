...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, augment_pipe=None, E=None, D_reg=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        super().__init__()
        self.device = device
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    ...
                    Ereg_logits = self.D_reg(context)
                    loss_Ereg = torch.nn.functional.softplus(-Ereg_logits)  # -log(sigmoid(gen_logits))
                    loss_Ereg.mean().mul(gain).backward(retain_graph=True)
            ...
            # Plot gradient flow
            training_stats.report('Grad/E_avg_grad', misc.get_grad(self.E.named_parameters()))
            training_stats.report('Grad/G_syn_avg_grad', misc.get_grad(self.G_synthesis.named_parameters()))
            training_stats.report('Grad/G_map_avg_grad', misc.get_grad(self.G_mapping.named_parameters()))
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if self.encode:
                    ...
                    z_Ereg = torch.randn_like(context, device=self.device, requires_grad=True)
                    Ereg_logits = self.D_reg(z_Ereg)
                    loss_Ereg = torch.nn.functional.softplus(-Ereg_logits)  # -log(sigmoid(gen_logits))
                    loss_Ereg.mean().mul(gain).backward(retain_graph=True)
                else:
