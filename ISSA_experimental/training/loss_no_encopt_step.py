        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    ...
                    gen_context = self.run_E(gen_img, sync=(sync and not do_Gpl))
                    gen_logits = self.run_D(gen_img, gen_context, sync=False)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                if self.encode:
                    ...
                    gen_context = self.run_E(gen_img, sync=False)
                    gen_logits = self.run_D(gen_img, gen_context, sync=False)  # Gets synced by loss_Dreal.
                else:
