    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                if self.encode:
                    context, ssize = self.run_E(real_img, sync=(sync and not do_Gpl))
        ...
        if do_Gpl:
            ...
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                if self.encode:
                    context, ssize = self.run_E(real_img, sync=sync)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                if self.encode:
                    context, ssize = self.run_E(real_img, sync=False)
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if self.encode:
                    context, ssize = self.run_E(real_img, sync=sync)
                    real_logits = self.run_D_shuffle(real_img_tmp, context, ssize, sync=sync)
                else:
